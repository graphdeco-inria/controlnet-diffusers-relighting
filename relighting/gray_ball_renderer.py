...
class GrayBallRenderer:
    ...
    def __post_init__(self):
        # Create mask
        mask = cv2.circle(
            np.zeros((self.resolution*4, self.resolution*4, 3), dtype=np.uint8),
            (self.resolution//2*4, self.resolution//2*4),
            self.resolution//2*4,
            (255, 255, 255),
            -1
        )
        self.mask = F.avg_pool2d(torch.from_numpy(mask/255).permute(2, 0, 1).float().mean(dim=0).nan_to_num(0.0)[None], 4)[0]
    ...
    def render_onto(self, bg, light_dir, padding=30):
        assert bg.ndim == 4
        # put the bottom half of the mask to 0
        # self.mask[self.resolution//2:] = 0.0
